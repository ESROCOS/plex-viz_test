...
INTERFACEVIEW=viz_test_iv.aadl
grep "version => \"2" viz_test_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view viz_test_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f odometry.zip && zip odometry odometry/* && cd $OLDPWD
